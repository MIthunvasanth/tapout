...
Ships the Slice 2.6 capture-quality fixes. **0.1.1 was published with only Slice 2.5** (SessionEnd auto-capture) due to a tag ordering mishap during release — the retag that pointed at Slice 2.6 code failed to upload to PyPI because 0.1.1 already existed. 0.1.2 is the release with the below Slice 2.6 changes actually on PyPI.
...
- 84 (up from 54 in 0.1.0).

## 0.1.1

> This release was published to PyPI with only Slice 2.5 content; see 0.1.2 for the Slice 2.6 additions.

Auto-capture on `/exit` now works reliably on Windows. Closes [#1](https://github.com/MIthunvasanth/tapout/issues/1).
